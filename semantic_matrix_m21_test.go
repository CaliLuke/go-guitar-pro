...
func TestSemanticMatrixM21SharedValidation(t *testing.T) {
	runSemanticMatrixM21SharedValidation(newSemanticMatrixRun(t))
}

func runSemanticMatrixM21SharedValidation(run *semanticMatrixRun) {
	t := run.t
	tests := []struct {
		name     string
		code     string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !m21HasScoreDiagnosticAt(diagnostics, test.code, test.location) {
			...
			if !m21HasExportEntryAt(preflight, wantCode, test.location) {
...
func TestSemanticMatrixM21AuthoredAndDerivedDistinction(t *testing.T) {
	runSemanticMatrixM21AuthoredAndDerivedDistinction(newSemanticMatrixRun(t))
}

func runSemanticMatrixM21AuthoredAndDerivedDistinction(run *semanticMatrixRun) {
	t := run.t
	tests := []struct {
		name   string
		code   string
...
func TestSemanticMatrixM21FinalizeContracts(t *testing.T) {
	runSemanticMatrixM21FinalizeContracts(newSemanticMatrixRun(t))
}

func runSemanticMatrixM21FinalizeContracts(run *semanticMatrixRun) {
	t := run.t
	for _, pickup := range []bool{false, true} {
		t.Run(fmt.Sprintf("pickup=%t", pickup), func(t *testing.T) {
			song := m21TimingSong(pickup)
			run.Field("Song.Anacrusis", song.Anacrusis, pickup)
			authored := m21Snapshot(song, true)
			if err := FinalizeSong(song); err != nil {
				t.Fatal(err)
			}
			if got := m21Snapshot(song, true); got != authored {
			...
			if second := m21Snapshot(song, false); second != first {
...
func TestSemanticMatrixM21ReadOnlyAndFileContracts(t *testing.T) {
	runSemanticMatrixM21ReadOnlyAndFileContracts(newSemanticMatrixRun(t))
}

func runSemanticMatrixM21ReadOnlyAndFileContracts(run *semanticMatrixRun) {
	t := run.t
	song := m21ReadOnlySong(t)
	...
	wantSong := m21Snapshot(song, false)
	wantOptions := m21Snapshot(options, false)
	assertReadOnly := func(operation string) {
		t.Helper()
		if got := m21Snapshot(song, false); got != wantSong {
			t.Fatalf("%s mutated the song", operation)
		}
		if got := m21Snapshot(options, false); got != wantOptions {
	...
	wantStrictSong := m21Snapshot(strictSong, false)
	wantStrictOptions := m21Snapshot(strict, false)
	directory := t.TempDir()
	...
	if got := m21Snapshot(strictSong, false); got != wantStrictSong {
		t.Fatal("failed strict file export mutated the song")
	}
	if got := m21Snapshot(strict, false); got != wantStrictOptions {
		t.Fatal("failed strict file export mutated options")
	}
}

func m21TimingSong(pickup bool) *Song {
	quarter := defaultBeat()
...
func m21AssertFinalizedPaths(run *semanticMatrixRun, song *Song) {
	t := run.t
	t.Helper()
...
func m21ReadOnlySong(t *testing.T) *Song {
	t.Helper()
	song := semanticExportProbeSong(t)
...
// m21Snapshot is a deterministic, non-JSON observation of the complete value
// graph. It dereferences pointers, walks slices and maps, and sees unexported
// ScoreTime terms through their primitive fields.
func m21Snapshot(value any, authoredOnly bool) string {
	var output strings.Builder
	m21WriteSnapshot(&output, reflect.ValueOf(value), authoredOnly)
	return output.String()
}

func m21WriteSnapshot(output *strings.Builder, value reflect.Value, authoredOnly bool) {
	if !value.IsValid() {
	...
	case reflect.Interface, reflect.Pointer:
		...
		output.WriteByte('&')
		m21WriteSnapshot(output, value.Elem(), authoredOnly)
	case reflect.Struct:
		output.WriteString(value.Type().String())
		output.WriteByte('{')
		for index := 0; index < value.NumField(); index++ {
			field := value.Type().Field(index)
			if authoredOnly && m21DerivedField(value.Type(), field.Name) {
				continue
			}
			output.WriteString(field.Name)
			output.WriteByte(':')
			m21WriteSnapshot(output, value.Field(index), authoredOnly)
			output.WriteByte(';')
		}
		output.WriteByte('}')
	case reflect.Slice:
		...
		for index := 0; index < value.Len(); index++ {
			m21WriteSnapshot(output, value.Index(index), authoredOnly)
			output.WriteByte(';')
		}
		output.WriteByte(']')
	case reflect.Array:
	...
	case reflect.Map:
		...
		for iterator.Next() {
			...
			m21WriteSnapshot(&item, iterator.Key(), authoredOnly)
			item.WriteByte(':')
			m21WriteSnapshot(&item, iterator.Value(), authoredOnly)
...
func m21DerivedField(owner reflect.Type, name string) bool {
	switch owner {
...
func m21HasScoreDiagnosticAt(diagnostics []ScoreDiagnostic, code string, location ScoreLocation) bool {
...
func m21HasExportEntryAt(report ExportReport, code string, location ScoreLocation) bool {
