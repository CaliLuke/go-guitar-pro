...
func TestBeatTimerGraceSourceLimit(t *testing.T) {
	raw := string(conformanceBarreGPIF(t, mustStringNumberExport(t, beatTimerSong(t))))
	raw = strings.Replace(raw, `<Beat id="0">`, `<Beat id="0"><GraceNotes>BeforeBeat</GraceNotes><Timer>99</Timer>`, 1)
	source := conformanceGPIFArchive(t, raw)
	...
	for _, d := range result.Diagnostics {
		if d.Code == "GPIF.Beat.Timer.GraceUnsupported" {
			found = true
	...
	if !found {
		t.Fatal("grace timer silently lost")
	}
	if _, err = ParseWithOptions(source, ParseOptions{Strict: true}); err == nil {
		t.Fatal("strict accepted grace timer loss")
	}
	requireAlphaTabConformance(t)
	var sourceFacts, targetFacts []beatTimerFact
	readAlphaTabOracleFacts(t, "--beat-timer", writeConformanceFixture(t, source), &sourceFacts)
	readAlphaTabOracleFacts(t, "--beat-timer", writeConformanceFixture(t, mustStringNumberExport(t, result.Song)), &targetFacts)
	count := func(facts []beatTimerFact) int {
		n := 0
		for _, fact := range facts {
			if fact.Show && fact.Milliseconds != nil && *fact.Milliseconds == 99 {
				n++
			}
		}
		return n
	}
	if count(sourceFacts) != 1 || count(targetFacts) != 0 {
		t.Fatalf("grace limit source=%#v target=%#v", sourceFacts, targetFacts)
	}
}
