...
func TestConformanceDurations(t *testing.T) {
	runConformanceDurations(newConformanceRun(t))
}

func runConformanceDurations(run *conformanceRun) {
	t := run.t
	...
	if diagnostic := conformanceSourceAuditDiagnosticByCode(unknownTupletContext.diagnostics, "Binary.Duration.Tuplet.Unsupported"); diagnostic == nil || diagnostic.Kind != ParseDiagnosticUnsupportedFeature {
	...
	for _, normalized := range []struct {
		...
		t.Run(normalized.name, func(t *testing.T) {
			song := conformanceTimingSingleBeatSong(t, normalized.duration)
...
func TestConformanceExactTiming(t *testing.T) {
	runConformanceExactTiming(newConformanceRun(t))
}

func runConformanceExactTiming(run *conformanceRun) {
	t := run.t
	...
	run.Derived("MeasureHeader.ExactStart", conformanceTimingTimes(song.MeasureHeaders), [][2]int64{{960, 1}, {4800, 1}, {8640, 1}})
	...
	run.Derived("Measure.ExactStart", conformanceTimingMeasureTimes(measures), [][2]int64{{960, 1}, {4800, 1}, {8640, 1}})
...
func conformanceTimingSingleBeatSong(t *testing.T, duration Duration) *Song {
	t.Helper()
...
func conformanceTimingTimes(headers []MeasureHeader) [][2]int64 {
...
func conformanceTimingMeasureTimes(measures []Measure) [][2]int64 {
...
func cloneTimingSong(source Song) Song {
	clone := source
	...
	for trackIndex := range clone.Tracks {
		clone.Tracks[trackIndex].Measures = cloneTimingMeasures(source.Tracks[trackIndex].Measures)
		...
		for staffIndex := range clone.Tracks[trackIndex].Staves {
			clone.Tracks[trackIndex].Staves[staffIndex].Measures = cloneTimingMeasures(source.Tracks[trackIndex].Staves[staffIndex].Measures)
...
func cloneTimingMeasures(source []Measure) []Measure {
...
type conformanceTimingWireRhythm struct {
	noteValue string
	dotCount  int
	tupletNum int
	tupletDen int
...
func extractTimingRhythm(t *testing.T, data []byte) conformanceTimingWireRhythm {
	t.Helper()
	...
	var result conformanceTimingWireRhythm
	inRhythms := false
	inFirstRhythm := false
	for {
