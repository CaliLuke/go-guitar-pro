...
func TestGP8PercussionUsesEveryResolvedStaff(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			song := ticket32PercussionSong(t, test.regular, test.grace, ticket32RepeatedLineCounts(len(test.regular), 5), nil)
			...
			wire := ticket32WireDocument(t, gpif)
			if got := ticket32WireOutputs(wire); !slices.Equal(got, test.wantMIDIs) {
				t.Fatalf("resolved wire output MIDI = %v, want %v", got, test.wantMIDIs)
			}
			if got := ticket32WireInputs(wire); !slices.Equal(got, test.wantInputs) {
			...
			staffs := ticket32AlphaStaves(t, alpha, 0)
			...
			if got := ticket32AlphaMainMIDIs(staffs); !slices.Equal(got, ticket32SortedRegularMIDIs(test.regular)) {
				t.Fatalf("AlphaTab main-note MIDI = %v, want %v", got, ticket32SortedRegularMIDIs(test.regular))
			}
		})
	}
}
...
func TestGP8PercussionKeepsCustomIdentityAndTrackOwnership(t *testing.T) {
	...
	song := ticket32PercussionSong(t, [][]int16{{93}, {92}, {36}}, nil, []int{5, 5, 5}, custom)
	...
	ticket32AddFollowingPitchedTrack(t, song)
	...
	wire := ticket32WireDocument(t, readM12GPIF(t, data))
	flat := ticket32FlatArticulations(wire.Tracks.Tracks[0].InstrumentSet.Elements.Elements)
	...
	if os.Getenv("ALPHATAB_CONFORMANCE") == "1" {
		...
		if len(tracks) != 2 || len(ticket32AlphaStaves(t, alpha, 0)) != 3 || ticket32AlphaStaves(t, alpha, 1)[0]["percussion"] != false {
			t.Fatalf("AlphaTab track ownership = %#v", tracks)
		}
		if got := ticket32AlphaMainMIDIs(ticket32AlphaStaves(t, alpha, 0)); !slices.Equal(got, []int{36, 56, 56}) {
...
func TestGP8PercussionKeepsReorderedCustomTables(t *testing.T) {
	...
	for _, order := range [][]int{{0, 1}, {1, 0}} {
		...
		t.Run(name, func(t *testing.T) {
			...
			song := ticket32PercussionSong(t, [][]int16{{int16(custom[0].InputMIDINumbers[0])}, {int16(custom[1].InputMIDINumbers[0])}, {36}}, nil, []int{5, 5, 5}, custom)
			data, report, err := ExportWithReport(song, ExportFormatGP8, ExportOptions{LossPolicy: ExportLossPolicy{RequirePreservation: true}})
			if err != nil || len(report.Entries) != 0 || len(data) == 0 {
				t.Fatalf("strict export = %d bytes, %#v, %v", len(data), report.Entries, err)
			}
...
func TestGP8PercussionUsesCompatibilityAuthorityForResources(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "replaced legacy slice", configure: func(track *Track) {
			track.Measures = ticket32MeasuresWithMIDIs(track.Staves[0].Measures, []int16{42})
		}, wantOutput: []int{36, 42}},
		{name: "replaced canonical staff after clearing legacy", configure: func(track *Track) {
			track.Measures = nil
			track.Staves[0].Measures = ticket32MeasuresWithMIDIs(track.Staves[0].Measures, []int16{46})
		}, wantOutput: []int{36, 46}},
	} {
		t.Run(test.name, func(t *testing.T) {
			song := ticket32PercussionSong(t, [][]int16{{38}, {36}}, nil, []int{5, 5}, nil)
			...
			if got := ticket32WireOutputs(ticket32WireDocument(t, readM12GPIF(t, data))); !slices.Equal(got, test.wantOutput) {
...
func TestGP8PercussionRejectsMissingArticulationResource(t *testing.T) {
	song := ticket32PercussionSong(t, [][]int16{{38}}, nil, []int{5}, nil)
	builder := gp8Builder{
		song:            song,
...
func TestGP8PercussionLineCountPolicy(t *testing.T) {
	for _, lines := range [][]int{{1, 1}, {5, 5}} {
		t.Run(strings.Join([]string{"equal", string(rune('0' + lines[0]))}, "-"), func(t *testing.T) {
			song := ticket32PercussionSong(t, [][]int16{{38}, {36}}, nil, lines, nil)
			data, report, err := ExportWithReport(song, ExportFormatGP8, ExportOptions{LossPolicy: ExportLossPolicy{RequirePreservation: true}})
			if err != nil || len(report.Entries) != 0 || len(data) == 0 {
				t.Fatalf("strict export = %d bytes, %#v, %v", len(data), report.Entries, err)
			}
			if os.Getenv("ALPHATAB_CONFORMANCE") == "1" {
				for staffIndex, staff := range ticket32AlphaStaves(t, readAlphaTabScore(t, writeConformanceFixture(t, data)), 0) {
	...
	for _, lines := range [][]int{{5, 1}, {1, 5}} {
		t.Run(strings.Join([]string{"different", string(rune('0' + lines[0])), string(rune('0' + lines[1]))}, "-"), func(t *testing.T) {
			...
			if os.Getenv("ALPHATAB_CONFORMANCE") == "1" {
				for _, staff := range ticket32AlphaStaves(t, readAlphaTabScore(t, writeConformanceFixture(t, data)), 0) {
...
func ticket32MeasuresWithMIDIs(source []Measure, midis []int16) []Measure {
	measure := source[0]
...
func ticket32PercussionSong(t *testing.T, regular [][]int16, grace map[int]int16, lines []int, custom []PercussionArticulation) *Song {
	t.Helper()
	song := m11Song(t)
	track := &song.Tracks[0]
...
func ticket32AddFollowingPitchedTrack(t *testing.T, song *Song) {
	t.Helper()
	pitchedSong := m11Song(t)
...
func ticket32WireDocument(t *testing.T, data []byte) gpifDocument {
	t.Helper()
...
func ticket32FlatArticulations(elements []gpifElement) []gpifArticulation {
...
func ticket32WireOutputs(document gpifDocument) []int {
	flat := ticket32FlatArticulations(document.Tracks.Tracks[0].InstrumentSet.Elements.Elements)
...
func ticket32WireInputs(document gpifDocument) []int {
	flat := ticket32FlatArticulations(document.Tracks.Tracks[0].InstrumentSet.Elements.Elements)
...
func ticket32AlphaStaves(t *testing.T, score any, trackIndex int) []map[string]any {
	t.Helper()
...
func ticket32AlphaMainMIDIs(staves []map[string]any) []int {
	var result []int
...
func ticket32SortedRegularMIDIs(staves [][]int16) []int {
	var result []int
...
func ticket32RepeatedLineCounts(count, value int) []int {
