...
func TestConformanceMasterBars(t *testing.T) {
	runConformanceMasterBars(newConformanceRun(t))
}

func runConformanceMasterBars(run *conformanceRun) {
	t := run.t
	song := conformanceMeasureSong(t)
	...
	run.Wire("gpifMasterBar.Repeat", []conformanceMeasureWireRepeat{wires.masterBars[0].repeat, wires.masterBars[1].repeat, wires.masterBars[2].repeat, wires.masterBars[3].repeat}, []conformanceMeasureWireRepeat{
	...
	neutral := extractMeasureWire(t, percussionData)
	run.Wire("gpifBar.Clef", neutral.clefs[0], "Neutral")
}

func conformanceMeasureDirectionSignMember(value DirectionSign) string {
	return []string{
...
func TestConformanceAuthorityAndBoundaries(t *testing.T) {
	runConformanceAuthorityAndBoundaries(newConformanceRun(t))
}

func runConformanceAuthorityAndBoundaries(run *conformanceRun) {
	t := run.t
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			source := conformanceMeasureGPIF(test.time, test.repeat, test.ending)
...
func conformanceMeasureSong(t *testing.T) *Song {
	t.Helper()
	song := semanticValidPitchedGP8Song(t)
	song.Tracks[0].Settings.Notation = true
...
type conformanceMeasureWireKey struct {
	mode  string
	count int
}

type conformanceMeasureWireRepeat struct {
	start string
	end   string
	count int
}

type conformanceMeasureWireMasterBar struct {
	key              conformanceMeasureWireKey
	time             string
	sectionLetter    string
	sectionText      string
	repeat           conformanceMeasureWireRepeat
...
func extractMeasureWire(t *testing.T, data []byte) conformanceMeasureWireScore {
	...
	var result conformanceMeasureWireScore
	var path []string
	for {
...
func conformanceMeasureGPIF(time, repeat, endings string) string {
	repeatXML := ""
	if repeat != "" {
