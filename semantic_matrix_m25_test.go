...
func TestSemanticMatrixM25PublicAPIEnums(t *testing.T) {
	runSemanticMatrixM25PublicAPIEnums(newSemanticMatrixRun(t))
}

func runSemanticMatrixM25PublicAPIEnums(run *semanticMatrixRun) {
	m25AssertExportEnums(run)
	m25AssertParseDiagnosticEnums(run)
	m25AssertScoreDiagnosticEnums(run)
}

func m25AssertExportEnums(run *semanticMatrixRun) {
	t := run.t
	...
	run.Enum(
		...
		m25ExportEntry(t, PreflightExport(normalized, ExportFormatGP8, ExportOptions{}), "gp8.normalize.beat-dynamic").Disposition,
	...
	run.Enum(
		...
		m25ExportEntry(t, PreflightExport(omitted, ExportFormatGP8, ExportOptions{}), "gp8.omit.writer").Disposition,
		ExportDispositionOmitted,
	)

	run.Enum(
		"ExportDisposition.ExportDispositionRejected",
		m25ExportEntry(t, PreflightExport(valid, ExportFormat(99), ExportOptions{}), "export.reject.target").Disposition,
		ExportDispositionRejected,
	)
}

func m25AssertParseDiagnosticEnums(run *semanticMatrixRun) {
	t := run.t
	tests := []struct {
		member        string
		...
		},
		{
			...
			code: "GPIF.Beat.Fadding.Lossy", defaultReject: true,
			mutate: func(source string) string {
				return insertFirstGPIFObjectChild(t, source, "<Beats>", "</Beat>", "<Fadding>FadeOut</Fadding>")
			},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.member, func(t *testing.T) {
			...
			diagnostic := m25ParseDiagnostic(t, result.Diagnostics, test.code)
			...
			if !errors.As(selectedErr, &strictErr) || m25ParseDiagnostic(t, strictErr.Diagnostics, test.code).Kind != test.kind {
...
func m25AssertScoreDiagnosticEnums(run *semanticMatrixRun) {
	tests := []struct {
		member string
	...
	for _, test := range tests {
		run.t.Run(test.member, func(t *testing.T) {
			...
			test.mutate(song)
			diagnostic := m25ScoreDiagnostic(t, ValidateSong(song), test.code)
			run.Enum("ScoreDiagnosticKind."+test.member, diagnostic.Kind, test.kind)
		})
	}
}

func m25ExportEntry(t *testing.T, report ExportReport, code string) *ExportReportEntry {
	t.Helper()
...
func m25ParseDiagnostic(t *testing.T, diagnostics []ParseDiagnostic, code string) *ParseDiagnostic {
	t.Helper()
...
func m25ScoreDiagnostic(t *testing.T, diagnostics []ScoreDiagnostic, code string) *ScoreDiagnostic {
	t.Helper()
	for index := range diagnostics {
		if diagnostics[index].Code == code {
			return &diagnostics[index]
