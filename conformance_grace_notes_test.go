...
func TestConformanceOrderedGraceExport(t *testing.T) {
	runConformanceOrderedGraceExport(newConformanceRun(t))
}
...
func runConformanceOrderedGraceExport(run *conformanceRun) {
	...
	track.Measures[0].Voices = []Voice{{Beats: []Beat{
		{
			...
			Notes: []Note{
				conformanceGraceNote(5, 1, []GraceEffect{
					conformanceGrace(2, DurationSixteenth, MinVelocity+VelocityIncrement*4, false, 0, GraceEffectTransitionNone, false),
					conformanceGrace(3, DurationThirtySecond, Forte, true, 1, GraceEffectTransitionSlide, true),
					conformanceGrace(4, 64, MinVelocity+VelocityIncrement*6, false, 2, GraceEffectTransitionHammer, false),
				}),
				conformanceGraceNote(1, 6, []GraceEffect{
					conformanceGrace(1, DurationThirtySecond, Forte, true, 1, GraceEffectTransitionNone, false),
				}),
				conformanceGraceNote(2, 2, []GraceEffect{
					conformanceGrace(1, DurationSixteenth, MinVelocity+VelocityIncrement*4, false, 0, GraceEffectTransitionNone, false),
					conformanceGrace(3, 64, MinVelocity+VelocityIncrement*6, false, 2, GraceEffectTransitionNone, false),
				}),
			},
		},
		{Duration: defaultDuration(), Status: BeatStatusRest},
	}}, {Beats: []Beat{
		{Duration: defaultDuration(), Status: BeatStatusNormal, Dynamics: Forte, Notes: []Note{conformanceGraceNote(3, 3, []GraceEffect{conformanceGrace(1, DurationThirtySecond, Forte, false, 0, GraceEffectTransitionNone, false)})}},
		{Duration: defaultDuration(), Status: BeatStatusRest},
	}}}
	...
	run.Preserved("GraceEffect.Duration", conformanceGraceDurations(wantPrimary), []uint8{DurationSixteenth, DurationThirtySecond, 64})
	run.Normalized("GraceEffect.Fret", conformanceGraceFrets(wantPrimary), []int8{2, 3, 4})
	run.Preserved("GraceEffect.ExactFret", conformanceGraceExactFrets(wantPrimary), []Fret{2, 3, 4})
	...
	run.Preserved("GraceEffect.PercussionArticulation", conformanceGraceArticulations(wantPrimary), []int{0, 0, 0})
	run.Preserved("GraceEffect.HasPercussionArticulation", conformanceGraceArticulationPresence(wantPrimary), []bool{false, false, false})
	run.Preserved("GraceEffect.IsDead", conformanceGraceDead(wantPrimary), []bool{false, true, false})
	run.Preserved("GraceEffect.IsOnBeat", conformanceGraceOnBeat(wantPrimary), []bool{false, true, false})
	run.Preserved("GraceEffect.Sequence", conformanceGraceSequences(wantPrimary), []uint8{0, 1, 2})
	run.Normalized("GraceEffect.Transition", conformanceGraceTransitions(wantPrimary), []GraceEffectTransition{GraceEffectTransitionNone, GraceEffectTransitionSlide, GraceEffectTransitionHammer})
	run.Preserved("GraceEffect.Velocity", conformanceGraceVelocities(wantPrimary), []int16{MinVelocity + VelocityIncrement*4, Forte, MinVelocity + VelocityIncrement*6})
	...
	run.Field("GraceEffect.Duration", conformanceGraceDurations(gotPrimary), []uint8{DurationSixteenth, DurationThirtySecond, 64})
	run.Field("GraceEffect.Fret", conformanceGraceFrets(gotPrimary), []int8{2, 3, 4})
	run.Field("GraceEffect.ExactFret", conformanceGraceExactFrets(gotPrimary), []Fret{2, 3, 4})
	run.Field("GraceEffect.IsDead", conformanceGraceDead(gotPrimary), []bool{false, true, false})
	run.Field("GraceEffect.IsOnBeat", conformanceGraceOnBeat(gotPrimary), []bool{false, true, false})
	run.Dispatch("parseGPIFWithContext:b.GraceNotes", conformanceGraceOnBeat(gotPrimary), []bool{false, true, false})
	run.Field("GraceEffect.Sequence", conformanceGraceSequences(gotPrimary), []uint8{0, 1, 2})
	run.Field("GraceEffect.Transition", conformanceGraceTransitions(gotPrimary), []GraceEffectTransition{GraceEffectTransitionNone, GraceEffectTransitionSlide, GraceEffectTransitionHammer})
	...
	run.Field("GraceEffect.Velocity", conformanceGraceVelocities(gotPrimary), []int16{MinVelocity + VelocityIncrement*4, Forte, MinVelocity + VelocityIncrement*6})
...
func TestConformanceGraceAuthorityAndLoss(t *testing.T) {
	runConformanceGraceAuthorityAndLoss(newConformanceRun(t))
}

func runConformanceGraceAuthorityAndLoss(run *conformanceRun) {
	t := run.t
...
func TestConformanceBinaryGraceSource(t *testing.T) {
	runConformanceBinaryGraceSource(newConformanceRun(t))
}

func runConformanceBinaryGraceSource(run *conformanceRun) {
	t := run.t
...
func TestConformanceCombinedGraceEffects(t *testing.T) {
	runConformanceCombinedGraceEffects(newConformanceRun(t))
}

func runConformanceCombinedGraceEffects(run *conformanceRun) {
	t := run.t
...
func conformanceGraceNote(fret int16, guitarString int8, graces []GraceEffect) Note {
	return Note{
...
func conformanceGrace(fret int8, duration uint8, velocity int16, onBeat bool, sequence uint8, transition GraceEffectTransition, dead bool) GraceEffect {
	exact := Fret(fret)
...
func conformanceGraceDurations(graces []GraceEffect) []uint8 {
...
func conformanceGraceFrets(graces []GraceEffect) []int8 {
...
func conformanceGraceExactFrets(graces []GraceEffect) []Fret {
...
func conformanceGraceArticulations(graces []GraceEffect) []int {
...
func conformanceGraceArticulationPresence(graces []GraceEffect) []bool {
...
func conformanceGraceDead(graces []GraceEffect) []bool {
	result := make([]bool, len(graces))
	for index := range graces {
...
func conformanceGraceOnBeat(graces []GraceEffect) []bool {
	result := make([]bool, len(graces))
	for index := range graces {
...
func conformanceGraceSequences(graces []GraceEffect) []uint8 {
	result := make([]uint8, len(graces))
	for index := range graces {
...
func conformanceGraceTransitions(graces []GraceEffect) []GraceEffectTransition {
...
func conformanceGraceVelocities(graces []GraceEffect) []int16 {
...
type conformanceGraceWire struct {
	graceKinds      []string
...
func extractGraceWire(t *testing.T, data []byte) conformanceGraceWire {
	t.Helper()
	...
	var result conformanceGraceWire
