...
const conformanceOwnershipGPIF = `<?xml version="1.0" encoding="utf-8"?>
<GPIF>
...
func TestConformanceOwnershipImport(t *testing.T) {
	runConformanceOwnershipImport(newConformanceRun(t))
}

func runConformanceOwnershipImport(run *conformanceRun) {
	song, err := parseGPIF([]byte(conformanceOwnershipGPIF))
	if err != nil {
		run.t.Fatal(err)
	}
	...
	wire := extractOwnershipWire(run.t, conformanceOwnershipGPIF)
...
func TestConformanceCompatibilityAuthority(t *testing.T) {
	runConformanceCompatibilityAuthority(newConformanceRun(t))
}

func runConformanceCompatibilityAuthority(run *conformanceRun) {
	t := run.t
...
func TestConformanceBarCardinalityDiagnostics(t *testing.T) {
	runConformanceBarCardinalityDiagnostics(newConformanceRun(t))
}

func runConformanceBarCardinalityDiagnostics(run *conformanceRun) {
	t := run.t
	tests := []struct {
		name string
		bars string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			source := replaceOwnershipMasterBarRefs(conformanceOwnershipGPIF, test.bars)
...
type conformanceOwnershipWire struct {
	trackRefs   []string
	trackIDs    []string
	trackNames  []string
	staffCounts []int
...
func extractOwnershipWire(t *testing.T, source string) conformanceOwnershipWire {
	t.Helper()
	decoder := xml.NewDecoder(strings.NewReader(source))
	result := conformanceOwnershipWire{}
	var path []string
	var currentTrack = -1
	for {
...
func replaceOwnershipMasterBarRefs(source, refs string) string {
