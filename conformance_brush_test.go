...
func runConformanceBrush(run *conformanceRun) {
	...
	run.ClaimReport(claimSite("brush", "export", conformanceBrushCase, "arpeggio down with 117 exact ticks")).Report(conformanceBrushCase, reportCodes(report), []string{"gp8.omit.pan-automation-consumer", "gp8.omit.volume-automation-consumer", "gp8.normalize.track-name-page-policy", "gp8.normalize.source-version", "gp8.omit.track-use-rse", "gp8.omit.track-display-settings"})
