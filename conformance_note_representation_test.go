...
func TestConformanceNoteRepresentation(t *testing.T) {
	runConformanceNoteRepresentation(newConformanceRun(t))
}

func runConformanceNoteRepresentation(run *conformanceRun) {
	t := run.t
...
type conformanceNoteWireNotes struct {
	ids           []string
	propertyNames []string
	ties          [][2]bool
}

func extractNotes(t *testing.T, data []byte) conformanceNoteWireNotes {
	t.Helper()
	...
	var result conformanceNoteWireNotes
	inNotes := false
	for {
...
func TestConformanceNoteValidation(t *testing.T) {
	runConformanceNoteValidation(newConformanceRun(t))
}

func runConformanceNoteValidation(run *conformanceRun) {
	t := run.t
