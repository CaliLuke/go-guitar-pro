...
func TestConformanceBackingAssetsAndSyncPoints(t *testing.T) {
	runConformanceBackingAssetsAndSyncPoints(newConformanceRun(t))
}

func runConformanceBackingAssetsAndSyncPoints(run *conformanceRun) {
	t := run.t
	gpif := conformanceBackingValidGPIF()
	wire := decodeBackingWire(t, []byte(gpif))
	...
	data := conformanceBackingArchive(t, gpif, map[string][]byte{
	...
	t.Run("external metadata is retained without fetching", func(t *testing.T) {
		external := strings.ReplaceAll(conformanceBackingValidGPIF(), "<Source>Local</Source>", "<Source>External</Source>")
		...
		externalResult, err := ParseWithOptions(conformanceBackingArchive(t, external, map[string][]byte{"Content/Assets/a.bin": []byte("unused")}), ParseOptions{Strict: true})
		if err != nil {
			t.Fatal(err)
		}
...
func TestConformanceBackingSourceDiagnostics(t *testing.T) {
	runConformanceBackingSourceDiagnostics(newConformanceRun(t))
}

func runConformanceBackingSourceDiagnostics(run *conformanceRun) {
	t := run.t
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			gpif: conformanceBackingGPIF("not-an-integer", "Content/Assets/audio.bin"),
			code: "GPIF.BackingTrack.FramePadding.Invalid",
		},
		{
			name: "missing embedded bytes",
			gpif: conformanceBackingGPIF("-22050", "Content/Assets/missing.bin"),
			code: "GPIF.BackingTrack.EmbeddedFile.Reference",
		},
		{
			name:    "empty embedded bytes",
			gpif:    conformanceBackingGPIF("-22050", "Content/Assets/empty.bin"),
		...
		},
		{
			...
			gpif: conformanceBackingGPIF("-22050", ""),
			code: "GPIF.BackingTrack.EmbeddedFile.Reference",
		},
		{
			name: "broken asset id",
			gpif: strings.Replace(conformanceBackingGPIF("0", "Content/Assets/audio.bin"), "<AssetId>asset-a</AssetId>", "<AssetId>missing</AssetId>", 1),
			code: "GPIF.BackingTrack.AssetId.Reference",
		},
		{
			name: "missing asset id",
			gpif: strings.Replace(conformanceBackingGPIF("0", "Content/Assets/audio.bin"), "<AssetId>asset-a</AssetId>", "<AssetId></AssetId>", 1),
			code: "GPIF.BackingTrack.AssetId.Reference",
		},
		{
			name: "duplicate asset id",
			gpif: strings.Replace(conformanceBackingGPIF("0", "Content/Assets/audio.bin"), "</Assets>", `<Asset id="asset-a"><EmbeddedFilePath>Content/Assets/other.bin</EmbeddedFilePath></Asset></Assets>`, 1),
			code: "GPIF.Asset.DuplicateID",
		},
		{
			name: "empty asset id",
			gpif: strings.Replace(conformanceBackingGPIF("0", "Content/Assets/audio.bin"), `id="asset-a"`, `id=""`, 1),
			code: "GPIF.Asset.EmptyID",
		},
		{
			name: "padding overflow",
			gpif: conformanceBackingGPIF("9223372036854775808", "Content/Assets/audio.bin"),
			code: "GPIF.BackingTrack.FramePadding.Invalid",
		},
		{
			name: "missing padding",
			gpif: strings.Replace(conformanceBackingGPIF("0", "Content/Assets/audio.bin"), "<FramePadding>0</FramePadding>", "", 1),
			code: "GPIF.BackingTrack.FramePadding.Invalid",
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			result, err := ParseWithOptions(conformanceBackingArchive(t, test.gpif, test.entries), ParseOptions{Strict: true})
			var strictErr *StrictParseError
			if !errors.As(err, &strictErr) {
				t.Fatalf("strict parse = %#v, %v, want StrictParseError", result, err)
			}
			diagnostic := conformanceBackingFindParseCode(result, test.code)
	...
	disabled := strings.Replace(conformanceBackingGPIF("0", "Content/Assets/missing.bin"), "<Enabled>true</Enabled>", "<Enabled>false</Enabled>", 1)
	disabled = strings.Replace(disabled, "<AssetId>asset-a</AssetId>", "<AssetId></AssetId>", 1)
	disabledResult, disabledErr := ParseWithOptions(conformanceBackingArchive(t, disabled, nil), ParseOptions{Strict: true})
	if disabledErr != nil {
	...
	for _, code := range []string{"GPIF.BackingTrack.AssetId.Reference", "GPIF.BackingTrack.EmbeddedFile.Reference"} {
		if diagnostic := conformanceBackingFindParseCode(disabledResult, code); diagnostic != nil {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			gpif := conformanceBackingReplaceOnce(t, conformanceBackingValidGPIF(), test.old, test.new)
			result, err := ParseWithOptions(conformanceBackingArchive(t, gpif, validEntries), ParseOptions{Strict: true})
			var strictErr *StrictParseError
			diagnostic := conformanceBackingFindParseCode(result, test.code)
...
func TestConformanceValidationAndExportPolicy(t *testing.T) {
	runConformanceValidationAndExportPolicy(newConformanceRun(t))
}

func runConformanceValidationAndExportPolicy(run *conformanceRun) {
	t := run.t
	...
	for _, code := range []string{"gp8.normalize.sync-point-frame-authority", "gp8.normalize.sync-point-position-authority"} {
		entries := conformanceBackingExportEntries(conflictReport, code)
...
func conformanceBackingGPIF(framePadding, embeddedPath string) string {
...
func conformanceBackingValidGPIF() string {
	return `<?xml version="1.0" encoding="utf-8"?>
<GPIF>
...
type conformanceBackingWireDocument struct {
	BackingTrack *struct {
...
func decodeBackingWire(t *testing.T, data []byte) conformanceBackingWireDocument {
	t.Helper()
	var result conformanceBackingWireDocument
...
func conformanceBackingProgrammaticSong(t *testing.T) *Song {
	t.Helper()
	zero, err := NewBarPosition(0, 1)
	if err != nil {
		t.Fatal(err)
...
func conformanceBackingExportEntries(report ExportReport, code string) []ExportReportEntry {
...
func conformanceBackingReplaceOnce(t *testing.T, source, old, replacement string) string {
	t.Helper()
...
func conformanceBackingArchive(t *testing.T, gpif string, entries map[string][]byte) []byte {
	t.Helper()
...
func conformanceBackingFindParseCode(result *ParseResult, code string) *ParseDiagnostic {
	if result == nil {
		return nil
	}
