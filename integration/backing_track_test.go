...
<GPIF>
  ...
  <Assets>
	...
	if first.BarPosition.Ratio().Numerator() != 0 || first.AudioFrame != guitarpro.AudioFrame(0) {
	...
	if second.AudioFrame != guitarpro.AudioFrame(79200) {
...
func TestGP8AudioTrackFixturePreservesSyncData(t *testing.T) {
	song, err := guitarpro.ParseFile(filepath.Join("..", "testdata", "gp8", "canon-audio-track.gp"))
	if err != nil {
		t.Fatal(err)
	}
	if song.BackingTrack == nil {
