...
func runSemanticMatrixM19BackingAssetsAndSyncPoints(run *semanticMatrixRun) {
	...
	run.Field("Song.BackingTrack", backing != nil, true)
	run.Field("BackingTrack.Name", backing.Name, "Selected audio")
	run.Field("BackingTrack.Source", backing.Source, "Local")
	run.Field("BackingTrack.AssetID", backing.AssetID, "asset-b")
	run.Field("BackingTrack.OriginalFilePath", backing.OriginalFilePath, "/selected/b.wav")
	run.Field("BackingTrack.OriginalFileSHA1", backing.OriginalFileSHA1, "sha-b")
	run.Field("BackingTrack.EmbeddedFilePath", backing.EmbeddedFilePath, "Content/Assets/b.bin")
	run.Field("BackingTrack.AudioData", backing.AudioData, selectedAudio)
	run.Field("BackingTrack.FramePadding", backing.FramePadding, int64(-22050))
	run.Field("BackingTrack.Enabled", backing.Enabled, true)
	if len(result.Song.SyncPoints) != 2 {
		t.Fatalf("sync points = %#v, want two", result.Song.SyncPoints)
	}
	run.Field("Song.SyncPoints", len(result.Song.SyncPoints), 2)
	...
	run.Field("SyncPoint.Bar", []int{result.Song.SyncPoints[0].Bar, result.Song.SyncPoints[1].Bar}, []int{0, 1})
	run.Field("SyncPoint.Position", []float64{result.Song.SyncPoints[0].Position, result.Song.SyncPoints[1].Position}, []float64{0, 1})
	run.Field("SyncPoint.BarPosition", []BarPosition{result.Song.SyncPoints[0].BarPosition, result.Song.SyncPoints[1].BarPosition}, []BarPosition{zero, one})
	run.Field("SyncPoint.BarOccurrence", []int{result.Song.SyncPoints[0].BarOccurrence, result.Song.SyncPoints[1].BarOccurrence}, []int{0, 2})
	run.Field("SyncPoint.FrameOffset", []int64{result.Song.SyncPoints[0].FrameOffset, result.Song.SyncPoints[1].FrameOffset}, []int64{0, 44100})
	run.Field("SyncPoint.AudioFrame", []AudioFrame{result.Song.SyncPoints[0].AudioFrame, result.Song.SyncPoints[1].AudioFrame}, []AudioFrame{0, 44100})
	run.Field("SyncPoint.MediaTimeMS", []float64{result.Song.SyncPoints[0].MediaTimeMS, result.Song.SyncPoints[1].MediaTimeMS}, []float64{500, 1500})
	run.Field("SyncPoint.ModifiedTempo", []float64{result.Song.SyncPoints[0].ModifiedTempo, result.Song.SyncPoints[1].ModifiedTempo}, []float64{120.5, 90.25})
	run.Field("SyncPoint.OriginalTempo", []float64{result.Song.SyncPoints[0].OriginalTempo, result.Song.SyncPoints[1].OriginalTempo}, []float64{100, 100.75})
	run.Field("SyncPoint.Linear", []bool{result.Song.SyncPoints[0].Linear, result.Song.SyncPoints[1].Linear}, []bool{false, true})
	run.Field("SyncPoint.Visible", []bool{result.Song.SyncPoints[0].Visible, result.Song.SyncPoints[1].Visible}, []bool{true, false})
