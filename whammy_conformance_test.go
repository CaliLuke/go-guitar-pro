...
func TestParseBinaryWhammyPreservesDipsAndHolds(t *testing.T) {
	fixtures, want := whammyConformanceBinaryWhammyCases()
...
func TestAlphaTabBinaryWhammyProjection(t *testing.T) {
	...
	for _, fixture := range fixtures {
		t.Run(fixture, func(t *testing.T) {
			...
			for index, expected := range want {
				beat := whammyConformanceNormalizedBeat(t, score, barOffset+index)
				if !reflect.DeepEqual(whammyConformanceBendPoints(beat["whammy"]), expected) {
...
func whammyConformanceBinaryWhammyCases() ([]string, [][]BendPoint) {
	return []string{
...
func TestAlphaTabGP3WhammyOracleLimitation(t *testing.T) {
	...
	for bar := 8; bar <= 11; bar++ {
		...
		if alphaWhammy := whammyConformanceNormalizedBeat(t, alphaScore, bar)["whammy"]; alphaWhammy != nil {
...
func TestAlphaTabWhammyCorpusConformance(t *testing.T) {
	...
	alphaScores, err := readCorpusOracleScores(fixtures)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGP8WhammyExportPreservesSupportedCurves(t *testing.T) {
	...
	for name, points := range curves {
		t.Run(name, func(t *testing.T) {
			...
			conformanceCurveSetCurve(song, "whammy", &BendEffect{Points: slices.Clone(points)})
...
func TestAlphaTabGP8WhammyExportProjection(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			got := simplifyBendPoints(whammyConformanceBendPoints(whammyConformanceNormalizedBeat(t, score, 0)["whammy"]))
...
func TestWhammyProjectionAdaptersExposeBeatCurves(t *testing.T) {
	...
	beat := whammyConformanceNormalizedBeat(t, normalizeGoScore(song), 0)
	want := []BendPoint{{Position: 0}, {Position: 6, Value: -4}, {Position: 12}}
	if got := whammyConformanceBendPoints(beat["whammy"]); !slices.Equal(got, want) {
...
func TestGP8WhammyExportLossPolicyIsExact(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			song := conformanceCurveSong(t)
			conformanceCurveSetCurve(song, "whammy", &BendEffect{Points: slices.Clone(test.points)})
			songBefore := conformanceContractSnapshot(song, false)
			...
			exactBefore := conformanceContractSnapshot(exact, false)
			...
			assertWhammyConformanceReadOnly(t, song, songBefore, exact, exactBefore)
		})
	}
}

func assertWhammyConformanceReadOnly(t *testing.T, song *Song, wantSong string, options ExportOptions, wantOptions string) {
	t.Helper()
	if conformanceContractSnapshot(song, false) != wantSong || conformanceContractSnapshot(options, false) != wantOptions {
		t.Fatal("whammy preflight/export mutated its Song or ExportOptions input")
	}
}

func whammyConformanceNormalizedBeat(t *testing.T, score any, bar int) map[string]any {
	t.Helper()
...
func whammyConformanceBendPoints(value any) []BendPoint {
	items, _ := value.([]any)
	result := make([]BendPoint, 0, len(items))
	for _, item := range items {
		point := item.(map[string]any)
		result = append(result, BendPoint{Position: uint8(whammyConformanceInteger(point["position"])), Value: int8(whammyConformanceInteger(point["value"]))})
	}
	return result
}

func whammyConformanceInteger(value any) int64 {
