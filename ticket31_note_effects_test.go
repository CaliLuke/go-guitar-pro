...
func TestTicket31GPIFHammerAndTapOriginsRemainDistinct(t *testing.T) {
...
func TestTicket31GPIFPickSlidesRetainDirection(t *testing.T) {
	for _, test := range []struct {
		flags string
		want  SlideType
...
func TestTicket31GPIFVibratoStrengthRemainsDistinct(t *testing.T) {
	for _, test := range []struct {
		wire string
		want NoteVibrato
...
func TestTicket31GPIFAccentKindRemainsDistinct(t *testing.T) {
	for _, test := range []struct {
		wire int
		want NoteAccent
...
func TestTicket31GP8HammerAndTapOriginsRoundTrip(t *testing.T) {
	tests := []struct {
		name       string
		effect     NoteEffect
		properties []string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			data := ticket31StrictEffectExport(t, test.effect)
...
func TestTicket31GP8PickSlideEquivalenceClass(t *testing.T) {
	for _, test := range []struct {
		name   string
		slides []SlideType
		flags  string
		...
		t.Run(test.name, func(t *testing.T) {
			...
			data := ticket31StrictEffectExport(t, effect)
			wire := extractM11WireNote(t, data)
...
func TestTicket31GP8NoteVibratoEquivalenceClass(t *testing.T) {
	for _, test := range []struct {
		name       string
...
func TestTicket31GP8NoteAccentEquivalenceClass(t *testing.T) {
	for _, test := range []struct {
		name       string
		accent     NoteAccent
		normal     bool
...
func TestTicket31NoteAccentAuthorityPolicy(t *testing.T) {
	effect := defaultNoteEffect()
	effect.Accent = NoteAccentTenuto
	effect.HeavyAccentuatedNote = true
	song := ticket31EffectSong(t, effect)
...
func TestTicket31RejectsInvalidNoteEffectEnums(t *testing.T) {
	tests := []struct {
		name string
		set  func(*NoteEffect)
		code string
...
func TestTicket31ResolvedHammerAndLinkedSlideEquivalence(t *testing.T) {
	staff := Staff{
...
func TestTicket31AlphaTabConsumesExportedNoteEffectKinds(t *testing.T) {
	...
	data := ticket31StrictSongExport(t, song)
...
func ticket31StrictEffectExport(t *testing.T, effect NoteEffect) []byte {
	t.Helper()
	return ticket31StrictSongExport(t, ticket31EffectSong(t, effect))
}

func ticket31StrictSongExport(t *testing.T, song *Song) []byte {
	t.Helper()
...
func ticket31EffectSong(t *testing.T, effect NoteEffect) *Song {
	t.Helper()
	song := m11Song(t)
...
func TestAlphaTabTicket31NoteEffectCorpusClasses(t *testing.T) {
	requireAlphaTabConformance(t)
	fixtures := []string{
	...
	oracleScores, err := readM23OracleScores(fixtures)
	if err != nil {
		t.Fatal(err)
	}
	var remaining []string
	for _, fixture := range fixtures {
		result, parseErr := ParseWithOptions(ticket31ReadFile(t, fixture), ParseOptions{})
		if parseErr != nil {
		...
		for _, difference := range semanticDifferences(goScore, alphaScore) {
			semanticPath := m23SemanticPath(difference.Path, goScore, alphaScore)
			if ticket31NoteEffectDifferencePath(semanticPath) {
...
func ticket31NoteEffectDifferencePath(path string) bool {
...
func ticket31ReadFile(t *testing.T, path string) []byte {
	t.Helper()
